...
import MusicEntryCard from './MusicEntryCard'
          ...
          return (
            ...
            >
              ...
              {item.type === 'text' ? (
                ...
                onClick={() => openTextLightbox(item.content as string, item.author, `${getDrawingIndex(item.entryId)}.png`)}>
                ...
                // v2.0: Music entry
                <MusicEntryCard
                  music={item.content as MusicRecommendation}
                  author={item.author}
                  timestamp={item.timestamp}
                  colorIndex={colorIndex}
                  drawingIndex={getDrawingIndex(item.entryId)}
                  groupInfo={groupInfo}
                  approvalStatus={approvalStatus}
                  showDeleteButtons={showDeleteButtons}
                  isAdmin={isAdmin}
                  onApprove={() => handleApprove(item.entryId)}
                  onDelete={() => handleDelete(item.entryId)}
                />
              ) : null /* v2.0: removed image support */}
            </motion.div>
          )
        })}
      </div>
